...
【ongoing】加载缓慢的问题 notes 也有 头秃
【ongoing】window resize 的时候有抖动 加防抖
学习知识的时候一定要有需求, 没有需求就创造需求

**问题**
焦点控制出现冲突  
使用了 v-focus 让第一个输入框聚焦; 使用 Enter 键手动切换焦点; 
为啥死循环了...

【extension】模糊搜索 -- 单词存在变形, 将最接近的结果用列表呈现;
【extension】在页面写一个外部搜索 google bing baidu
【extension】音乐左边可以有一个小方块, 拖动小方块进入全屏音乐
