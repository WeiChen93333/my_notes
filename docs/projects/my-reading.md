### 项目描述
#### 三、部署上线
> 网上搜了很多文章和教程, 从购买服务器和配置开始, 文章里的设置和我遇到的不一样. 一方面是因为官方更新或版本升级, 界面和操作不一样了, 然后一些以前要做的配置现在不需要了; 另一方面是好多文章虽然是最近发布的, 但是内容却很古老, 今年发布的文章, 截图中出现的时间却是 18 年...因此列出项目环境如下:
- 部署时间: 20200606 - 20200607
- 本地环境: windows
...
- 释义模式
  - 阅读区单击单词查询; 记忆卡点击查询 (点击正面单词页, 则显示背面释义页); 搜索框输入单词查询(点击单词信息展示区的红线, 弹出查词输入框); 右键菜单选择 "查询释义"
  - 显示内容为词典中该单词的释义
- 例句模式
...
- 查词跳转历史
  将查询的单词放入 vuex-state 中定义的数组, 通过右键菜单的 "返回" 回到上一个查询的单词(记忆卡模块不支持)
- 单词集-记忆卡不支持例句模式, 打开该模块时, 将强行切换为释义模式
...
<!-- [considering] size 的默认值要不要在 SentenceDisplay 加载后发送给 MainPart 呢 
分页组件的左右点击需要优化, 同时, 查询新单词时, pagenum 需要归位
-->
